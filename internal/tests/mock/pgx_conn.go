...
func PgxConnMock(ctrl *gomock.Controller, mockPgData *Postgres) *mockgenPostgres.MockQuery {
	...
	mockDb.EXPECT().QueryRow(gomock.Any(), postgresRepository.GetUserByLoginQuery, mockUserData.AdminUser1.Login).Return(PgMockRow{
	...
	mockDb.EXPECT().QueryRow(gomock.Any(), postgresRepository.GetUserByLoginQuery, mockUserData.BannedUser1.Login).Return(PgMockRow{
